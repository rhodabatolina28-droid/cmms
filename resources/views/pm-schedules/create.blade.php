...
<div class="page-container">
    <div class="premium-card">
        <h1 class="form-title">Create PM Schedule</h1>
        <p class="form-subtitle">Set up an automated preventive maintenance schedule. Each run assigns one user per asset, prioritizing oldest assets first.</p>

        <form method="POST" action="{{ route('pm-schedules.store') }}" id="scheduleForm">
            @csrf

            <div class="form-section">
                <div class="form-group">
                    <label>Schedule Name <span class="required">*</span></label>
                    <input type="text" name="schedule_name" value="{{ old('schedule_name') }}" required
                        class="input-focus"
                        placeholder="e.g. All Divisions PM Cycle">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="division_filter" class="select-full">
                            ...
                            <option value="RID" {{ old('division_filter') === 'RID' ? 'selected' : '' }}>Research & Information Division</option>
                            <option value="AD" {{ old('division_filter') === 'AD' ? 'selected' : '' }}>Administrative Division</option>
                            <option value="FMD" {{ old('division_filter') === 'FMD' ? 'selected' : '' }}>Financial & Management Division</option>
                            <option value="COA" {{ old('division_filter') === 'COA' ? 'selected' : '' }}>Commission on Audit</option>
                            <option value="CMD" {{ old('division_filter') === 'CMD' ? 'selected' : '' }}>Conciliation & Mediation Division</option>
                            <option value="VAD" {{ old('division_filter') === 'VAD' ? 'selected' : '' }}>Voluntary Arbitration Division</option>
                            <option value="WRED" {{ old('division_filter') === 'WRED' ? 'selected' : '' }}>Workplace Relations Enhancement Division</option>
                            <option value="OED" {{ old('division_filter') === 'OED' ? 'selected' : '' }}>Office of the Executive Director</option>
                        </select>
                        <div class="hint">Leave as "All Divisions" to cycle through all divisions automatically, oldest asset first.</div>
                    </div>

                    <div class="form-group">
                        <label>Frequency <span class="required">*</span></label>
                        <select name="frequency" required class="select-full">
                            <option value="Monthly" {{ old('frequency') === 'Monthly' ? 'selected' : '' }}>Monthly — every month</option>
                            <option value="Quarterly" {{ old('frequency') === 'Quarterly' ? 'selected' : '' }}>Quarterly — every 3 months</option>
                            <option value="Semi-annual" {{ old('frequency') === 'Semi-annual' ? 'selected' : '' }}>Semi-annual — every 6 months</option>
                            <option value="Annual" {{ old('frequency') === 'Annual' ? 'selected' : '' }}>Annual — every year</option>
                        </select>
            ...
            <div class="form-actions">
                <a href="{{ route('pm-schedules.index') }}" class="btn-cancel">
