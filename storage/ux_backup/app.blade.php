...
<head>
    ...
    <style nonce="{{ $cspNonce }}">
        ...
        .logo-section { text-align: center; border-bottom: 1px solid rgba(255,255,255,0.1); padding: 20px 10px; }
        .sidebar-logo-img { width: 45px; height: auto; margin-bottom: 10px; }
        .sidebar-title { font-family: 'Arial Black', sans-serif; font-size: 18px; margin: 0; color: white; letter-spacing: 1px; font-weight: 900; }
        .sidebar-role { font-family: Arial, sans-serif; font-size: 12px; margin-top: 2px; opacity: 0.9; color: white; text-transform: uppercase; font-weight: bold; }
        ...
        .sidebar-footer { padding: 20px; border-top: 1px solid rgba(255,255,255,0.1); opacity: 0.6; font-size: 12px; color: white; }

        /* Backdrop */
        #sidebarBackdrop { display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:99998; -webkit-tap-highlight-color:transparent; }
...
<body style="font-family: Arial, sans-serif; -webkit-font-smoothing: antialiased;">
    ...
    @auth
        ...
        <aside class="sidebar" id="sidebar">
            ...
            <nav>
                <!-- DASHBOARD (ALL) -->
                <a href="{{ route(Auth::user()->dashboardRouteName()) }}" class="nav-link {{ request()->routeIs('dashboard.*') ? 'active' : '' }}">
                ...
                @if(Auth::user()->role === 'super_admin')
                    <!-- SUPER ADMIN MODULES -->
                    <a href="{{ route('super_admin.users') }}" class="nav-link {{ request()->routeIs('super_admin.users') ? 'active' : '' }}">
                        <i class="fa-solid fa-users-gear"></i> User Management
                    </a>
                    <a href="{{ route('super_admin.audit_logs') }}" class="nav-link {{ request()->routeIs('super_admin.audit_logs') ? 'active' : '' }}">
                        <i class="fa-solid fa-shield-halved"></i> Audit Logs
                    </a>
                    <a href="{{ route('ict.index') }}" class="nav-link {{ request()->routeIs('ict.index') ? 'active' : '' }}">
                        <i class="fa-solid fa-clipboard-list"></i> All Requests
                    </a>
                    <a href="{{ route('super_admin.inventory') }}" class="nav-link {{ request()->routeIs('super_admin.inventory*') ? 'active' : '' }}">
                        <i class="fa-solid fa-boxes-stacked"></i> Asset Registry
                    </a>
                    <a href="{{ route('requisitions.index') }}" class="nav-link {{ request()->routeIs('requisitions.*') ? 'active' : '' }}">
                        <i class="fa-solid fa-box"></i> My Parts Requests
                    </a>
                    <a href="{{ route('pm-schedules.index') }}" class="nav-link {{ request()->routeIs('pm-schedules.*') ? 'active' : '' }}">
                        <i class="fa-solid fa-calendar-clock"></i> PM Schedules
                    </a>
                @elseif(Auth::user()->role === 'admin')
                    <!-- DIVISION ADMIN MODULES -->
                    <a href="{{ route('personnel.index') }}" class="nav-link {{ request()->routeIs('personnel.*') ? 'active' : '' }}">
                        <i class="fa-solid fa-users"></i> Manage Personnel
                    </a>
                    ...
                    @endif
                    <a href="{{ route('ict.index') }}" class="nav-link {{ request()->routeIs('ict.*') ? 'active' : '' }}">
                        <i class="fa-solid fa-clipboard-list"></i> {{ Auth::user()->department ? 'Department' : (Auth::user()->office ? 'Office' : 'Division') }} Requests
                    </a>
                    @if(Auth::user()->canProcessSupply())
                    <a href="{{ route('requisitions.index') }}" class="nav-link {{ request()->routeIs('requisitions.*') ? 'active' : '' }}">
                        <i class="fa-solid fa-truck"></i> Supply Workspace
                    </a>
                    @endif
                @elseif(Auth::user()->role === 'it')
                    <a href="{{ route('ict.index') }}" class="nav-link {{ request()->routeIs('ict.*') || request()->routeIs('maintenance.*') ? 'active' : '' }}">
                        <i class="fa-solid fa-screwdriver-wrench"></i> My Assigned Jobs
                    </a>
                    <a href="{{ route('pm.tasks') }}" class="nav-link {{ request()->routeIs('pm.tasks') ? 'active' : '' }}">
                        <i class="fa-solid fa-calendar-check"></i> PM Tasks
                    </a>
                    <a href="{{ route('requisitions.index') }}" class="nav-link {{ request()->routeIs('requisitions.*') ? 'active' : '' }}">
                        <i class="fa-solid fa-box"></i> My Parts Requests
                    </a>
                @elseif(Auth::user()->role === 'user')
                    <!-- END USER MODULES -->
                    <a href="{{ route('profile.assets') }}" class="nav-link {{ request()->routeIs('profile.assets') ? 'active' : '' }}">
                        <i class="fa-solid fa-laptop-medical"></i> My Assets
                    </a>
                    <a href="{{ route('ict.create') }}" class="nav-link {{ request()->routeIs('ict.create') ? 'active' : '' }}">
                        <i class="fa-solid fa-desktop"></i> ICT Request
                    </a>
    ...
    <script nonce="{{ $cspNonce }}">
        ...
        (function() {
            ...
            if (main) main.classList.add('no-transition');

            // On mobile, sidebar always starts hidden; on desktop restore saved state
            if (!isMobile) {
                try {
                    var saved = localStorage.getItem('cmms_sidebar_collapsed');
                    if (saved === 'true' && sidebar && main) {
        ...
        function toggleSidebar() {
            ...
            if (sidebar && main) {
                if (isMobile) {
                    ...
                    try {
                        localStorage.setItem('cmms_sidebar_collapsed', sidebar.classList.contains('collapsed'));
                    } catch(e) {}
                }
            }
        }
    ...
    <style nonce="{{ $cspNonce }}">
        ...
        @media (max-width: 767px) {
            ...
            .sidebar { left: -100% !important; width: min(280px, 85vw) !important; }
